...
def head():
    return lambda f_local__l_3: (
        (m_local__x_4)
        if (match(f_local__l_3, {"_a_a": ("m_local__x_4", "m_local_____none_5"),}))
        else (exit())
    )
...
def is_list_empty():
    return lambda f_local__l_6: (
        ({"True": None,})
        if (match(f_local__l_6, {"[]": None,}))
        else (
            (
                ({"False": None,})
                if (match(f_local__l_6, "m_local_____none_7"))
                else (exit())
            )
        )
    )
...
def main():
    return (
        (_o_i())(
            (
                (0)
                if (match((is_list_empty())(l()), {"True": None,}))
                else (
                    (
                        ((head())(l()))
                        if (match((is_list_empty())(l()), {"False": None,}))
                        else (exit())
                    )
                )
            )
